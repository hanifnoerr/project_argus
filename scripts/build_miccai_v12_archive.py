...
def _included_files(code_only: bool) -> list[Path]:
    ...
    individual = [
        ...
        ROOT / "experiments/gave2_ensemble/__init__.py",
        ROOT / "experiments/gave2_ensemble/data.py",
        ROOT / "experiments/gave2_ensemble/biomarkers.py",
        ROOT / "experiments/gave2_ensemble/biomarkers_v2.py",
        ROOT / "experiments/gave2_ensemble/preprocessing.py",
        ROOT / "experiments/gave2_ensemble/submission.py",
        ROOT / "experiments/gave2_ensemble/submission_v2.py",
        ROOT / "submission/GAVE2_R2V2_FFA_Residual_V12_Colab.ipynb",
        ROOT / "scripts/build_miccai_v12_archive.py",
    ]
    files = []
    for root in roots:
...
def _validate_local_import_closure(files: list[Path]) -> None:
    ...
    missing = sorted(
        str(available[module].relative_to(ROOT).as_posix())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a clean, self-auditing GAVE2 V12 archive.")
