...
from .model import build_model, teacher_corridor
from .utils import case_ids, sha256_file
...
def _predict_tta(model, features, teacher, mask, corridor, amp_dtype, use_tta: bool):
    import torch
...
def run_prediction(args: argparse.Namespace) -> dict[str, object]:
    ...
    namespace = f"gave2_v12_raw_{args.task}"
    ...
    if not all(path.exists() for path in checkpoint_paths):
        raise FileNotFoundError(f"Missing V12 checkpoints: {[str(path) for path in checkpoint_paths if not path.exists()]}")
    ...
    corridor_radius = int(loaded[0][0].config.corridor_radius)
    if any(int(model.config.corridor_radius) != corridor_radius for model, _ in loaded):
        raise RuntimeError("Fold checkpoints disagree on corridor radius")
    dataset = V12Dataset(
        args.data_root,
        args.split,
        args.task,
    ...
    for sequence, case_id in enumerate(pending, 1):
        ...
        predictions = [
            _predict_tta(loaded[index][0], features, teacher_tensor, mask, corridor, amp_dtype, args.tta)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict V12 OOF or validation residual probabilities.")
